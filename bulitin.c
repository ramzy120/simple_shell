...
void exit_func(char **argv, char **env, char **token_array)
{
	...
	{
		error_msg(2, 1, argv[0], token_array[0], "Illegal number: HBTN");
	...
	{
		error_msg(2, 1, argv[0], token_array[0], "Illegal number: -98");
		free_array(token_array);
		errno = 2;
		exit(errno);
	}
...
void cd_func(char **argv, char **env, char **token_array)
{
	...
	if (token_array[1] != NULL)
	{
		...
		{
			error_msg(2, cmd_count, argv[0], "can't cd to", token_array[1]);
		}
		return;
	}
	if (chdir(getcwd(dir, size)) == -1)
	{
		error_msg(2, cmd_count, argv[0], "can't cd to", token_array[0]);
	}
